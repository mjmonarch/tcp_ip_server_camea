...
class CameaService:
    ...
    Methods:
    send_image_found_response(conn, id, img, request, settings, lp, country) --> None
        Sends the response to the CAMEA DB Management Software query
    send_stab_image_data(id, dt_response, request, settings) --> None
        Sends the autogenerated stab images to the Camea Database
    send_image_data(id, dt_response, request, settings, img) --> None
    ...
    def send_image_found_response(conn, id: int, dt_response: datetime,
    ...
    def send_stab_image_data(id: int, dt_response: datetime,
                             request: dict, settings: dict) -> None:
        """
        Sends the autogenerated stab images to the Camea Database
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s2:
            s2.connect((settings['CAMEA_DB_IP'], settings['CAMEA_DB_PORT']))
    ...
    def send_image_data(id: int, dt_response: datetime,
                        request: dict, settings: dict, img: dict) -> None:
        """
        Sends the received from the Vidar DB image to the Camea Database
